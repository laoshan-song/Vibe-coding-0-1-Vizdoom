...
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/predict_position/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：能在目标移动时做出一定预判，而不是只盯着当前位置。
...
默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/predict_position/model_path.txt)。
