...
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/simpler_basic/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：不再随机乱动，已经会朝目标方向做明显修正。
...
推荐下一站：[`basic`](/home/laoshansong/vizdoom_project/scenarios/basic/README.md)

## 模型文件说明

默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/simpler_basic/model_path.txt)。
