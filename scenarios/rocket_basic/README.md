...
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/rocket_basic/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：会先对准再攻击，且不会因为武器反馈变化就完全崩掉。
...
默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/rocket_basic/model_path.txt)。
