...
```text
进入 health_gathering_supreme/
   |
   v
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/health_gathering_supreme/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：在更强压力下仍能保持找资源和续命的连续性。
...
推荐下一站：[`my_way_home`](/home/laoshansong/vizdoom_project/scenarios/my_way_home/README.md)

## 模型文件说明

默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/health_gathering_supreme/model_path.txt)。
