...
```text
进入 basic_notifications/
   |
   v
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/basic_notifications/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：能利用和 basic 类似的动作逻辑，表现稳定不过度抖动。
...
默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/basic_notifications/model_path.txt)。
