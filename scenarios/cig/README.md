...
运行 train.py
   |
   v
保存模型到 artifacts/scenarios/cig/ppo_model.zip
   |
   v
运行 play.py 观察行为
```
...
Vibe Check：能形成基础的持续作战行为，而不是几秒内就完全失控。
...
推荐下一站：[`deathmatch`](/home/laoshansong/vizdoom_project/scenarios/deathmatch/README.md)

## 模型文件说明

默认模型路径见 [`model_path.txt`](/home/laoshansong/vizdoom_project/scenarios/cig/model_path.txt)。
